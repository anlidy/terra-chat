...
`evals/` 提供无需外部服务的 smoke 验证、针对真实聊天文档语料的可重复检索基准，
以及显式启用的端到端答案评测。检索指标始终使用确定性 gold evidence；LLM judge
只用于可选的答案 faithfulness、correctness 和 completeness，不作为检索主指标。
...
每个 dataset/profile、strategy 和 rerank 组合固定写入
`<dataset>-<strategy>-latest.json` 与 `<dataset>-<strategy>-latest.md`，再次运行同一组合会
覆盖上一份报告，不会按时间戳累积文件。
...
full 英文集包含 35 个 case 和 22 份 PDF；full 中文集包含 30 个 case：15 个事实题、
10 个需要两组证据的信息整合题、5 个 corpus-unanswerable 题，共 125 份正负例文本。
加上 project 中英各 5 题，full 的语言分布为 40 EN / 35 ZH。TXT 在本地解码，
PDF/DOCX/XLSX/PPTX 才调用 LlamaCloud；三类语料仍共用生产切块、Embedding、写库和
检索流程。
...
- `--answer-model=<provider-id>/<model-id>`：用当前账号已配置的模型生成回答并运行同模型 judge，同时统计答案正确性、完整性、忠实度、引用、token、外部调用和已知模型价格下的成本。
...
- 15 个 RGB 中文事实样本和 10 个信息整合样本，每题保留 2 个正例和 3 个负例段落；
- 从未摄取其证据的后续 RGB 问题派生 5 个中文不可回答样本；
...
- 事实题从 `zh_refine.json` 按数字 `id` 取前 15 题；信息整合题从 `zh_int.json`
  取前 10 题并各保留两组证据；再从 `zh_refine.json` 的后续问题派生 5 个不可回答
  样本，但不把这些问题的 source passage 加入 corpus。
...
- `Document recall@5`：前 5 个结果是否出现任一 gold 文档，不要求已命中具体证据；
- `Gold-document coverage@5`：多文档题所需 gold 文档在前 5 个结果中的覆盖比例；
- `Evidence Recall@5`：前 5 个结果是否至少命中一条具体 gold 证据，即原 `Recall@5`；
- `Evidence coverage@5`：所有 gold evidence text（无文本时使用 gold page）的覆盖比例；
- `Context precision@5`：前 5 个返回 chunk 中命中具体 gold 证据的比例；
...
聚合或切片没有对应分母时，指标在 JSON 中为 `null`、Markdown 中为 `—`；例如纯
answerable 切片不伪造 false-retrieval rate，纯 unanswerable 切片也不把 evidence recall
显示成 0。

JSON 报告保留每个 case 的语言、题型、相关排名、召回数量、耗时、错误，以及 top-k 的
chunk/resource、文件、页码、内容预览、gold 相关性和各阶段分数。Markdown 失败表
直接列出 top-k 文件位置与分数，并按 language/category 输出切片表，便于区分语言、
任务和语料问题。JSON 和 Markdown
...
启用 `--answer-model` 时会额外生成 answer JSON 报告。引用 precision 衡量已生成引用中
正确 gold 文档的比例，并对重复引用去重计功；引用 recall 衡量所需 gold 文档的覆盖。
同一 judge 分别评分 faithfulness、与 `expectedAnswer` 的 correctness 和 completeness。
当前 answer 与 judge 使用同一模型，报告以 `judgeIndependence: same-model` 明示其偏乐观
风险。只有显式登记价格的模型才计算成本；当前 DeepSeek V4 Flash 按每百万 token
缓存命中输入 USD 0.0028、未命中输入 USD 0.14、输出 USD 0.28 估算，其他模型的
`pricingUsdPerMillionTokens` 与 `estimatedCostUsd` 为 `null`，不会套用错误价格。文件固定写入
`<dataset>-answer-latest.json`，同一数据集再次运行时覆盖上一份。
...
runner 使用 dataset/profile、strategy 和 rerank 模式生成固定的 `*-latest` 文件名，
因此中英文与各策略报告相互独立，同一组合的新报告覆盖旧报告。比较时以 JSON 为机器可读基线；报告中的
